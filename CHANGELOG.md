...
- `Frame::source_gcrs_position` - apparent GCRS [`Position`] of a source,
  scaled to the source's actual distance (or the NOVAS 1-GPC convention
  for stars without parallax). Use this as the `phase_center` argument to
  `uvw::uvw` and `Uvw::delay_rate`.

### Changed

- **`uvw::uvw` and `Uvw::delay_rate` now take `&Position`** for the phase
  center instead of a raw `[f64; 3]` in AU. The AU conversion is handled
  internally, consistent with how `station_pos` is already treated.
- **`Uvw::delay_rate_ns_per_s` renamed to `delay_rate`** and now returns
  a dimensionless rate (s/s) instead of ns/s. Callers that need ns/s
  can multiply by `1e9`.
- **`Frame::source_gcrs_direction` removed** - it returned a unit direction
  that was only usable for `delay_rate`, and passing it to `uvw::uvw`
  silently corrupted the result (see Fixed). Use `source_gcrs_position`
  for both.
...
- **`uvw::uvw` docs incorrectly claimed the phase-center magnitude was
  ...
  computes parallax from its magnitude. Passing a unit vector (as the
  removed `source_gcrs_direction` returned) places the source at 1 AU,
  corrupting `w` by `baseline² / AU` (~420 m on an 8000 km baseline). The
  parameter is now documented as a position; use `source_gcrs_position` to
  obtain it.
- **`los_to_xyz` / `xyz_to_los` docs incorrectly described the
  representation as `(\deltaφ, \deltaθ, \delta r)`** (spherical
  perturbation). The functions actually compute a Cartesian rotation onto
  the local (East, North, LOS) frame - the same UVW representation as
  `xyz_to_uvw` / `uvw_to_xyz`, just parameterized by `(lon, lat)` instead
  of `(ha, dec)`. Confirmed against the C source: `novas_xyz_to_uvw` is a
  one-line wrapper around `novas_xyz_to_los`.
...
- `Horizontal::offset(direction, distance)` - offset along a great-circle
  arc (wraps `novas_offset_by`). Zero distance is identity; at the poles
  returns an `Error` since longitude is undefined.
- `Horizontal::offset_by_sky(xel, el)` - sky-frame offset to great-circle
  conversion. Decomposes a cross-elevation / elevation pair into
  `(direction, distance)` and calls `offset`. Convenient for telescope
  raster scans and pointing-model corrections.
